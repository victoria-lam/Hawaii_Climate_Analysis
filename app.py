...
def dates_prcp():
    prcp_by_dates = session.query(Measurements.date, func.avg(Measurements.prcp))                    .filter(Measurements.date.between("2016-01-01", "2016-12-31"))                    .group_by(Measurements.date)                    .order_by(Measurements.date).all()
...
def tobs():
    results = session.query(Measurements.tobs)              .filter(Measurements.date.between("2016-01-01", "2016-12-31"))              .group_by(Measurements.date).all()
...
def start_date():
    results = session.query(func.min(Measurements.tobs), func.avg(Measurements.tobs), func.max(Measurements.tobs))              .filter(Measurements.date >= "2017-05-22").all()
    #convert to list
...
def date_range():
    results = session.query(func.min(Measurements.tobs), func.avg(Measurements.tobs), func.max(Measurements.tobs))           .filter(Measurements.date.between("2017-05-22", "2017-05-31")).all()
    #convert to list
